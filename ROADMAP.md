...
- [~] **Integration test suite runtime green** — gemma4 empty-response part done: `E_MultiTurnChat_Works` + the two `Gemma4EmptyChatProbeTests` guards pass reliably (isolation + serial). Added `tests/LMSupply.Integration.Tests/AssemblyInfo.cs` `DisableTestParallelization` (mirrors Generator.Tests) to stop concurrent llama-servers exhausting the 8GB GPU. NOT yet fully green: a *pre-existing* harness flake remains — rapid serial load/dispose of llama-server races on teardown (each test passes alone; ~1 of 4 back-to-back fails). Tracked separately: `ISSUE-...functional-gpu-rapid-load-dispose-teardown-flake.md`.
- [ ] **gemma4 registry thinking metadata** (follow-up, deferred) — gemma4-E2B behaves thinking-default-on, but `ThinkingEnabledByDefault` is advisory-only (never consumed for behavior) and only E2B is verified; adding the flag to gemma4 entries forces a family pack + version bump for zero runtime change. Needs E4B/26B/31B probe first. Issue: `ISSUE-...gemma4-registry-thinking-metadata-mismatch.md`.
